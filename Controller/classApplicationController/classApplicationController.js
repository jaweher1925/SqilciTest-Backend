...
module.exports = {
  createClassApplicant: async (req, res) => {
    try {
      ...
      return res.status(201).json({ message: 'Class applicant created successfully', data: savedApplicant });
    } catch (err) {
      return res.status(500).json({ message: 'Error creating class applicant', error: err.message });
    }
  },
  ...
  getAllClassApplicants: async (req, res) => {
    try {
      ...
      return res.status(500).json({ message: 'Error fetching class applicants', error: err.message });
    }
  },
  ...
  getClassApplicantById: async (req, res) => {
    try {
      ...
      return res.status(500).json({ message: 'Error fetching class applicant', error: err.message });
    }
  },
  ...
  updateClassApplicant: async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: 'Class applicant updated successfully', data: updatedApplicant });
    } catch (err) {
      return res.status(500).json({ message: 'Error updating class applicant', error: err.message });
    }
  },

  deleteClassApplicant: async (req, res) => {
    try {
      const deletedApplicant = await ClassApplicationModel.findByIdAndDelete(req.params.id);
      if (!deletedApplicant) {
        return res.status(404).json({ message: 'Class applicant not found' });
      }
      return res.status(200).json({ message: 'Class applicant deleted successfully' });
    } catch (err) {
      return res.status(500).json({ message: 'Error deleting class applicant', error: err.message });
    }
  }
};
